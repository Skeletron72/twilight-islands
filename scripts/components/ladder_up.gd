...
# Ladder leading back up to previous floor or surface.
...
func _update_prompt_text() -> void:
	...
	if DungeonManager.current_floor <= 1:
		prompt_label.text = "[E] На поверхность"
	else:
		prompt_label.text = "[E] Этаж %d" % (DungeonManager.current_floor - 1)
