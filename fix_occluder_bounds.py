...
for filepath in glob.glob('scenes/objects/trees/*.tscn'):
    ...
    # Extend the occluder down to the trunk base so it catches the player immediately!
    ...
    for i, line in enumerate(lines):
        ...
        if line.startswith('[node'):
            ...
            # Add explicit mask just in case
            if current_node == "OccluderArea":
                has_mask = False
                for j in range(i+1, min(i+4, len(lines))):
                    if lines[j].startswith('collision_mask'): has_mask = True
                    if lines[j].startswith('['): break
                if not has_mask:
                    out_lines.append('collision_mask = 1\n')
            continue
